...
if [ $STEP -eq 2 ]; then
	...
	sbatch --time=7-00:00:00 --cpus-per-task=1 --parsable -e $STD_ERR_OUT_DIR/%A_scan2_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_%x.out ${PIPELINE_DIR}/2_scan2.sh --project $PROJECT --results_dir $RESULTS_DIR --genome_version $ANNOVAR_GENOME_VERSION --script_dir $SCRIPT_DIR --sample_prefix $SAMPLE_PREFIX --normal_path $NORMAL_PATH --sample_string $TEMP_SAMPLES_STRING --vcf_path $VCF_PATH --std_err_out $STD_ERR_OUT_DIR --normal_name $NORMAL_SAMPLE_NAME --cross_dir $CROSS_SAMPLE_DIR	
	#sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
        #            -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
