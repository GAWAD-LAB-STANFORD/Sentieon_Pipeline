...
#
#	sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_step_1_%x.err -o $STD_ERR_OUT_DIR/%A_step_1_%x.out --time=5-00:00:00 --dependency=afterany:$( echo "${DEPENDENCIES}" ) --wrap "sh $PIPELINE_DIR/2_scan2.sh --step1 ${OPTIONS}"
#
#
#fi
...
if [ $STEP -eq 0 ]; then
	...
	if [ ! "$(ls -A "${SCAN2_RESULTS}")" ]; then
		...
		echo "***STARTING VARIANT CALLING NOW***"
                 DEPENDENCIES+=( $(sbatch --verbose --array=1-${TEMP_JOB_COUNT} --parsable -e ${STD_ERR_OUT_DIR}/%A_mmq1_sentieon_variant_call_scan2_%x.err -o ${STD_ERR_OUT_DIR}/%A_mmq1_sentieon_variant_call_scan2_%x.out $PIPELINE_DIR/scan2_germline_call.sh --results-dir $RESULTS_DIR --sample-prefix $SAMPLE_PREFIX --normal-path $NORMAL_BAM_PATH --ref $REF_FASTA --dbsnp $DBSNP_VCF --scan2-results $SCAN2_RESULTS --regions-bed $TARGETS_BED --sample_string $BAM_SAMPLE_STRING --mmq 1 --pipeline_dir $PIPELINE_DIR --bam_args echo ${BAM_ARGS}) )

		DEPENDENCIES+=( $(sbatch --verbose --array=1-${TEMP_JOB_COUNT} --parsable -e ${STD_ERR_OUT_DIR}/%A_mmq60_sentieon_variant_call_scan2_%x.err -o ${STD_ERR_OUT_DIR}/%A_mmq60_sentieon_variant_call_scan2_%x.out $PIPELINE_DIR/scan2_germline_call.sh --results-dir $RESULTS_DIR --sample-prefix $SAMPLE_PREFIX --normal-path $NORMAL_BAM_PATH --ref $REF_FASTA --dbsnp $DBSNP_VCF --scan2-results $SCAN2_RESULTS --regions-bed $TARGETS_BED --sample_string $BAM_SAMPLE_STRING --mmq 60 --pipeline_dir $PIPELINE_DIR --bam_args echo ${BAM_ARGS}) )


		### joint genotyping code block ends here ###

#                 DEPENDENCIES+=( $(sbatch --parsable -e ${STD_ERR_OUT_DIR}/%A_mmq1_sentieon_scan2_%x.err -o ${STD_ERR_OUT_DIR}/%A_sentieon_scan2_%x.out $PIPELINE_DIR/mmq1_scan2_sentieon.sh --results-dir $RESULTS_DIR --sample-prefix $SAMPLE_PREFIX --normal-path $NORMAL_BAM_PATH --ref $REF_FASTA --dbsnp $DBSNP_VCF --scan2-results $SCAN2_RESULTS --regions-bed $INTERVAL_LIST --cross_dir $CROSS_SAMPLE_DIR --bam_args echo ${BAM_ARGS}) )
#
#                DEPENDENCIES+=( $(sbatch --parsable -e ${STD_ERR_OUT_DIR}/%A_mmq60_sentieon_scan2_%x.err -o ${STD_ERR_OUT_DIR}/%A_sentieon_scan2_%x.out $PIPELINE_DIR/mmq60_scan2_sentieon.sh --results-dir $RESULTS_DIR --sample-prefix $SAMPLE_PREFIX --normal-path $NORMAL_BAM_PATH --ref $REF_FASTA --dbsnp $DBSNP_VCF --scan2-results $SCAN2_RESULTS --regions-bed $INTERVAL_LIST --cross_dir $CROSS_SAMPLE_DIR --bam_args echo ${BAM_ARGS}) )
#		echo DEPENDENCIES are $( IFS=$':'; echo "${DEPENDENCIES[*]}" )
#		echo DEPENDENCIES ALSO ARE: "${DEPENDENCIES[*]}" 
        fi
	echo dependencies are ${DEPENDENCIES}
	if [ ! -z $DEPENDENCIES ]; then
		echo OPTIONS ARE ${OPTIONS}
        	sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) --partition=cgawad -e $STD_ERR_OUT_DIR/%A_scan2_stepJ_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_stepJ_%x.out --time=5-00:00:00 --wrap "sh $PIPELINE_DIR/2_scan2.sh --stepJ ${OPTIONS}"
	else
		echo OPTIONS ARE ${OPTIONS}
		sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_scan2_stepJ_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_stepJ_%x.out --time=5-00:00:00 --wrap "sh $PIPELINE_DIR/2_scan2.sh --stepJ ${OPTIONS}"
...
if [ $STEP -eq 15 ]; then
	...
	if [ ! -f "$FILE" ]; then
		DEPENDENCIES+=( $(sbatch --parsable -e ${STD_ERR_OUT_DIR}/%A_mmq1_joint_gt_%x.err -o ${STD_ERR_OUT_DIR}/%A_mmq1_joint_gt_%x.out $PIPELINE_DIR/scan2_joint_genotyping.sh $RESULTS_DIR $REFERENCE_DIR $REF_FASTA $PROJECT 1 $SCAN2_RESULTS) )
		DEPENDENCIES+=( $(sbatch --parsable -e ${STD_ERR_OUT_DIR}/%A_mmq60_joint_gt_%x.err -o ${STD_ERR_OUT_DIR}/%A_mmq60_joint_gt_%x.out $PIPELINE_DIR/scan2_joint_genotyping.sh $RESULTS_DIR $REFERENCE_DIR $REF_FASTA $PROJECT 60 $SCAN2_RESULTS) )
	fi
	if [ ! -z $DEPENDENCIES ]; then
                sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) --partition=cgawad -e $STD_ERR_OUT_DIR/%A_scan2_step1_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_step1_%x.out --time=5-00:00:00 --wrap "sh $PIPELINE_DIR/2_scan2.sh --step1 ${OPTIONS}"
        else
                sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_scan2_step1_MAKE_PANEL_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_step1_MAKE_PANEL_%x.out --time=5-00:00:00 --job-name="step1" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step1 ${OPTIONS}"
...
if [ $STEP -eq 1 ]; then
	...
	DEPENDENCIES=( $(sbatch --time=7-00:00:00 -p cgawad scan2 run --joblimit 95 --snakemake-args " --keep-going --max-status-checks-per-second 0.1" --cluster 'sbatch -p cgawad -c {threads} --mem={resources.mem} -t 7-00:00:00 -o %logdir/slurm-%A.log') )
		...
		sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_scan2_step2_CALL_MUTS_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_step2_CALL_MUTS_%x.out --time=5-00:00:00 --dependency=afterany:$( echo "${DEPENDENCIES}" ) --job-name="step2" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step2 ${OPTIONS}"
	else
		sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_scan2_step2_CALL_MUTS_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_step2_CALL_MUTS_%x.out --time=5-00:00:00 --job-name="step2" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step2 ${OPTIONS}"
	fi
...
#
	...
	echo -e "sbatch --dependency=afterany:$( echo "${DEPENDENCIES}" ) -J $PROJECT \
			 $PIPELINE_DIR/2_scan2.sh --step3 ${OPTIONS[@]}
	fi"

	DEPENDENCIES="${DEPENDENCIES[-1]}"
	#PLEASE UNCOMMENT THIS LATER EVERYTHING IS BROKEN AND I HAVE TO CHECK IF ITS JUST THE CONFIG THATS MESSING UP
	##sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_scan2_step3_RESCUE_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_step3_RESCUE_%x.out --time=5-00:00:00 --dependency=afterany:$( echo "${DEPENDENCIES}" ) --job-name="step3" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step3 ${OPTIONS}"
...
#--mem={resources.mem} <--- put this back in cluster sbatch args if stuff breaks
	...
	echo -e "sbatch --dependency=afterany:$( echo "${DEPENDENCIES}" ) -J $PROJECT \
			 $PIPELINE_DIR/2_scan2.sh --step4 ${OPTIONS[@]}
	fi"


     
	sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_post_scan2_%x.err -o $STD_ERR_OUT_DIR/%A_post_scan2_%x.out --time=5-00:00:00 --dependency=afterany:$( echo "${DEPENDENCIES}" ) --job-name="step4" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step4 ${OPTIONS}"
...
if [ $STEP -eq 4 ];then
	...
	#tsv_extract saves the dataframes in the R objects as TSVs
		...
		echo "Running Scan2_tsv_extract.R now"
                Rscript ${PIPELINE_DIR}/Scan2_tsv_extract.R $RDA $SAMPLE_PATH
	...
	sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_post_scan2_%x.err -o $STD_ERR_OUT_DIR/%A_post_scan2_%x.out --time=5-00:00:00 --dependency=afterany:$( echo "${DEPENDENCIES}" ) --job-name="step5" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step5 ${OPTIONS}"
fi
...
#        sed '0,/#/{s/#//}' $MERGED_TSV > $MERGED_TSV
	...
	sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_post_scan2_%x.err -o $STD_ERR_OUT_DIR/%A_post_scan2_%x.out --time=5-00:00:00 --dependency=afterany:$( echo "${DEPENDENCIES}" ) --job-name="step6" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step6 ${OPTIONS}"
...
if [ $STEP -eq 6 ];then
	...
	sbatch --partition=cgawad -e $STD_ERR_OUT_DIR/%A_post_scan2_%x.err -o $STD_ERR_OUT_DIR/%A_post_scan2_%x.out --time=5-00:00:00 --dependency=afterany:$( echo "${DEPENDENCIES}" ) --job-name="step7" --wrap "sh $PIPELINE_DIR/2_scan2.sh --step7 ${OPTIONS}"
fi
...
#### OLD CODE ####
	...
	#should do a merge of all vcf files after finished annotating and doing sigprofiler, don't really see a problem with doing sigprofiler and annotate iteratively but also no reason can't do it at the end after merging 

	#get a header from the original vcf
#                sed '0,/CHROM/{s/CHROM/#CHROM/}' $MERGED_TSV > temp2.tsv
#                mv temp2.tsv $MERGED_TSV
#		sed '0,/##CHROM/{s/##CHROM/#CHROM/}' $MERGED_TSV > temp2.tsv
#                mv temp2.tsv $MERGED_TSV
#		grep '##' $GATK_VCF > temp2
#                grep -E "" $MERGED_TSV >> temp2
#                mv temp2 $MERGED_VCF
#
#	cd $RESULTS_DIR/$SCAN2_RESULTS/results
#	DEPENDENCIES+=( $(sbatch -c 2 --mem=32G -p cgawad --time=24:00:00 ${SCRIPT_DIR}/Scan2_SigProfiler.sh --project "${PROJECT}.tranche_${TRANCHE}" \
#                        --tsv $MERGED_TSV \
#                        --script_dir ${SCRIPT_DIR} --results_dir ${results} --project ${PROJECT}_merged) )
#	#annotating the file too cuz why not
#
#	ANNOTATED_MERGED_VCF=${MERGED_VCF}_scan2
#	DEPNDENCIES+=( $(sbatch -c 4 --mem=64G -p cgawad --time=24:00:00 --job-name="merged_annovar" --wrap="perl ${ANNOVAR_DIR}/table_annovar.pl $MERGED_VCF -vcfinput -operation g,f,f,f,f,f,f \
#                        ${ANNOVAR_DIR}/humandb -buildver $GENOME_VERSION \
#                        -out $ANNOTATED_MERGED_VCF -nastring . -remove -otherinfo \
#                        -protocol refGene,avsnp150,dbnsfp35c,clinvar_20190305,cosmic91_coding,cosmic91_noncoding,gnomad211_exome") )
