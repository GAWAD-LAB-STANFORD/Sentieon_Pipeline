...
ml biology samtools/1.6 bcftools/1.6 bedtools gatk/4.1.4.1
...
## this block of code is meant to downsample to 5 mil for exomes 500 mil for whole genomes, and compute preseq for miniseq runs
	...
	if [ $TOTAL_READS -ge 5000000 ] && [ $TARGETED -eq 1]; then
	   ...
	   BAM_NAME=${SAMPLE}_subsampled_exome.bam
	    ...
	    samtools view -b -L $TARGETS_BED ${BAM_NAME} > ${SAMPLE}${BAM_5M_SUFFIX}.targets.bam
	    $PRESEQ_TOOL_DIR/bam2mr -o ${SAMPLE}${BAM_5M_SUFFIX}.mr ${BAM_NAME}
	    $PRESEQ_TOOL_DIR/preseq gc_extrap -o ${SAMPLE}.gc_extrap.future_coverage_5M.tsv ${SAMPLE}${BAM_5M_SUFFIX}.mr
	    rm ${SAMPLE}${BAM_5M_SUFFIX}.targets.bam* ${SAMPLE}${BAM_5M_SUFFIX}.targets.mr
	...
	if [ $TOTAL_READS -le 5000000 ]; then
	    ml gsl/2.3
        ml biology bedtools/2.30.0
        samtools view -b -L $N25CHR_BED ${RECALIBRATED_BAM} > ${SAMPLE}.bqsr.marked.n25chr.bam
        #preseq is not working WITH THEIR OWN bam2mr TOOL! Using bed file as input for now, will ask Yakun if this
        #breaks everything
        #$PRESEQ_TOOL_DIR/bam2mr -o ${SAMPLE}.bqsr.mr ${RECALIBRATED_BAM}
	    #$PRESEQ_TOOL_DIR/bam2mr -L 10000 -o ${SAMPLE}.bqsr.mr ${SAMPLE}.bqsr.marked.n25chr.bam
        bedtools bamtobed -i ${SAMPLE}.bqsr.marked.n25chr.bam > ${SAMPLE}.bed 
        $PRESEQ_TOOL_DIR/preseq gc_extrap -o ${SAMPLE}.gc_extrap.future_coverage.tsv ${SAMPLE}.bed
...
#	    gatk --java-options "-XX:+UseParallelGC -XX:ParallelGCThreads=4 -Xmx31g -Xms31G" DownsampleSam \
#		-I ${RECALIBRATED_BAM} -O ${SAMPLE}${BAM_5M_SUFFIX} \
#		--PROBABILITY $FRACTION --VALIDATION_STRINGENCY SILENT \
#		--MAX_RECORDS_IN_RAM 5500000
#	    echo "5M downsample done"
...
#	    $PRESEQ_TOOL_DIR/bam2mr -o ${SAMPLE}${BAM_5M_SUFFIX}.mr ${SAMPLE}${BAM_5M_SUFFIX}
#	    $PRESEQ_TOOL_DIR/preseq gc_extrap -o ${SAMPLE}.gc_extrap.future_coverage_5M.tsv ${SAMPLE}${BAM_5M_SUFFIX}.mr
...
#	if [ $TOTAL_READS -le 5000000 ]; then
	    #samtools view -b -L $N25CHR_BED ${RECALIBRATED_BAM} > ${SAMPLE}.bqsr.marked.n25chr.bam
#	    $PRESEQ_TOOL_DIR/bam2mr -o ${SAMPLE}.bqsr.mr ${RECALIBRATED_BAM}
#	    $PRESEQ_TOOL_DIR/preseq gc_extrap -o ${SAMPLE}.gc_extrap.future_coverage.tsv ${SAMPLE}.bqsr.mr
#	    rm ${SAMPLE}.bqsr.mr
#	    if [ ! -f ${SAMPLE}.gc_extrap.future_coverage.tsv ]; then
#		echo "PreSeq encountered a problem and did not complete"
#	    else
