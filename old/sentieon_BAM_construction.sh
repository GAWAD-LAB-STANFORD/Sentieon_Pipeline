...
if [ $SKIP_TRIMMOMATIC -eq 0 ]; then
    ...
    R1_FASTQ=${SAMPLE}${R1_SUFFIX}
    R2_FASTQ=${SAMPLE}${R2_SUFFIX}
fi

echo "### Aligning fastqs ### - START: $(date)"
#-R "@RG\tID:$id\tPL:ILLUMINA\tLB:$lb\tSM:$sm"
RG='"'"@RG\tID:$SAMPLE\tPL;ILLUMINA\tLB:$SAMPLE\tSM:$SAMPLE"'"'
echo "Read group: $RG"

(sentieon bwa mem -R $RG \
...
sentieon driver -t $NUMBER_THREADS -r $REF_FASTA -i $REALIGNED_BAM \
    -q ${SAMPLE}_recal_data.table --algo Haplotyper -d $dbSNP $VARIANT_VCF
sentieon driver -t $NUMBER_THREADS -r $REF_FASTA -i $RECALIBRATED_BAM \
