#!/bin/bash
#
#SBATCH --job-name=test_submit_all
...
Run after demultiplexing: \n\t\
    sh ${PIPELINE_DIR}/test_submit_all.sh --fastq_dir /oak/stanford/groups/cgawad/MRD_project/ --project MRD_project \n\n\
Run with demultiplexing: \n\t\
    sh ${PIPELINE_DIR}/test_submit_all.sh --run_dir /oak/stanford/groups/cgawad/Illumina_Data/MiniSeq/191126_MN01236_0003_A000H2WWHT --fastq_dir /oak/stanford/groups/cgawad/MRD_project/ --project MRD_project \n\n\
...
#Have to put OPTIONS+= any arguments you want to continually be used through cycles of test_submit_all.sh
...
#Remember options from previous loops through submit_all.sh
...
if [ ! -z $SLURM_OPTIONS ]; then
    ...
    sbatch -J $PROJECT ${SLURM_OPTIONS[@]} \
        -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
        ${PIPELINE_DIR}/test_submit_all.sh ${OPTIONS[@]}
    exit 0
fi
...
#		    fi
	...
	if [ $STEP -eq 0 ]; then
		...
		-e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
		${PIPELINE_DIR}/test_submit_all.sh --step1 ${OPTIONS[@]}\n" >> $PIPELINE_STATUS
	    sbatch --dependency=afterok:${DEPENDENCIES[0]} -J $PROJECT \
		-e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
		${PIPELINE_DIR}/test_submit_all.sh --step1 ${OPTIONS[@]}
	...
	#Prep sample array for sentieon_BAM_construction.sh and setup to queue mutlipe paralell jobs into slurm for each sample    
	    ...
	    echo -e "\nsbatch --cpus-per-task $NUMBER_THREADS --parsable -e $STD_ERR_OUT_DIR/%A_%x.err -o $STD_ERR_OUT_DIR/%A_%x.out \
		...
		--array=1-${TEMP_JOB_COUNT} -p cgawad --cpus-per-task 16 --mem-per-cpu=8G ${PIPELINE_DIR}/1_sentieon_BAM_construction.sh \
...
#honestly i don't understand the point of the code below	     
#	    if [ $TEMP_ARRAY_START -le ${#SAMPLE_ARRAY[@]} ]; then
#		echo -e "\nsbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
#		    -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
#		    ${PIPELINE_DIR}/test_submit_all.sh --step1 --temp_array_start $TEMP_ARRAY_START ${OPTIONS[@]}\n" >> $PIPELINE_STATUS
#		sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
#		    -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
#		    ${PIPELINE_DIR}/test_submit_all.sh --step1 --temp_array_start $TEMP_ARRAY_START ${OPTIONS[@]}
#	    else
#		echo -e "\nsbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
#		    -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
#		    ${PIPELINE_DIR}/test_submit_all.sh --step2 ${OPTIONS[@]}\n" >> $PIPELINE_STATUS
		
#	    fi
		sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
		    -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
		    ${PIPELINE_DIR}/test_submit_all.sh --stepQC ${OPTIONS[@]}
	fi
fi
...
#	    echo "temp sample string is ${TEMP_SAMPLES_STRING}"
		...
		$RESULTS_DIR $SKIP_TRIMMOMATIC $SCRIPT_DIR $TOOLS_DIR $R1_SUFFIX $R2_SUFFIX $REF_FASTA $NUMBER_THREADS $TEMP_SAMPLES_STRING $DBSNP_VCF" >> $PIPELINE_STATUS
	    DEPENDENCIES+=( $(sbatch --parsable -e $STD_ERR_OUT_DIR/%A_%x.err -o $STD_ERR_OUT_DIR/%A_%x.out \
		--array=1-${TEMP_JOB_COUNT} -p cgawad --cpus-per-task 8 --mem=64G ${PIPELINE_DIR}/1.5_metrics_calc.sh \
...
if [ $STEP -eq 2 ]; then
	...
	sbatch --time=7-00:00:00 --cpus-per-task=1 --parsable -e $STD_ERR_OUT_DIR/%A_scan2_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_%x.out ${PIPELINE_DIR}/1_cell_scan2_submit.sh --project $PROJECT --results_dir $RESULTS_DIR --genome_version $ANNOVAR_GENOME_VERSION --script_dir $SCRIPT_DIR  --normal_path $NORMAL_PATH --sample_string $TEMP_SAMPLES_STRING --vcf_path $VCF_PATH --std_err_out $STD_ERR_OUT_DIR --normal_name $NORMAL_SAMPLE_NAME --cross_dir $CROSS_SAMPLE_DIR --sample_prefix $SAMPLE_PREFIX	
	#sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
        #            -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
        #            ${PIPELINE_DIR}/test_submit_all.sh --step3 ${OPTIONS[@]}
#--array=1-${TEMP_JOB_COUNT} 	
	###comment this after testing###
	exit
fi

if [ $STEP -eq 3 ]; then
	if [ -z $SKIP_GINKGO ]; then
		mkdir ${RESULTS_DIR}/ginkgo_outputs
		BAM_REGEX=".recalibrated_realigned_deduped_sorted.bam"
		sbatch -J $PROJECT \
			-e ${STD_ERR_OUT_DIR}/%A_ginkgo_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
			${PIPELINE_DIR}/2_ginkgo_cnv.sh --new_5M_folder $RESULTS_DIR/${PROJECT}_5M_Read_BAM_Files \
			--bam_dir $RESULTS_DIR --results_dir $RESULTS_DIR/${PROJECT}_Ginkgo_CNV_Analysis --bam_regex ${BAM_REGEX} 
	fi	
		
		
	DEPENDENCIES+=( $(sbatch -J $PROJECT --parsable \
...
if [ $STEP -eq 3 ]; then
        ...
        echo ${TEMP_SAMPLES_STRING}
        echo "dependencies are: ${DEPENDENCIES[*]}" 
        DEPENDENCIES+=( $(sbatch --cpus-per-task $NUMBER_THREADS --parsable -e $STD_ERR_OUT_DIR/%A_germline_variant_call.err -o $STD_ERR_OUT_DIR/%A_germline_variant_call.out \
        ...
        TEMP_ARRAY_START=$(($TEMP_ARRAY_START + $TEMP_ARRAY_INCREMENT))
            ...
            echo -e "\nsbatch --dependency=afterany:$( IFS=$':'; echo "$DEPENDENCIES" ) -J $PROJECT \
                -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
                ${PIPELINE_DIR}/test_submit_all.sh --step4 ${OPTIONS[@]}\n" >> $PIPELINE_STATUS
	...
	echo "Normal sample Sname is ${NORMAL_SAMPLE_NAME}"
	TEMP_ARRAY_INCREMENT=1000
	SAMPLE_ARRAY=( $(find ${RESULTS_DIR} -maxdepth 1 -name "${SAMPLE_PREFIX}*.deduped_sorted.bam" ! -name "*${NORMAL_SAMPLE_NAME}*" -exec basename {} \;) )
	JOB_COUNT=${#SAMPLE_ARRAY[@]} 
	TEMP_ARRAY_START=1
	...
	echo ${TEMP_SAMPLES_STRING}
	echo "dependencies are: ${DEPENDENCIES[*]}" 
	DEPENDENCIES+=( $(sbatch --cpus-per-task $NUMBER_THREADS --parsable -e $STD_ERR_OUT_DIR/%A_somatic_variant_call.err -o $STD_ERR_OUT_DIR/%A_somatic_variant_call.out \
	--array=1-${TEMP_JOB_COUNT} ${PIPELINE_DIR}/3_somatic_variant_calling.sh  \
	$RESULTS_DIR $REFERENCE_DIR $REF_FASTA $NUMBER_THREADS $NORMAL_SAMPLE_NAME $SAMPLE_PREFIX $TEMP_SAMPLES_STRING $DBSNP_VCF) )
...
if [ $STEP -eq 4 ]; then
	...
	if [ -n $NORMAL_SAMPLE_NAME ] && [ -n "$(find -name "${SAMPLE_PREFIX}*_variant.vcf")" ]; then
		...
		DEPENDENCIES+=( $(sbatch --cpus-per-task $NUMBER_THREADS --parsable -e $STD_ERR_OUT_DIR/%A_vcf_concat.err -o $STD_ERR_OUT_DIR/%A_vcf_concat.out ${PIPELINE_DIR}/4_vcf_concat.sh $RESULTS_DIR $SAMPLE_PREFIX) )
	fi
...
#		sentieon driver -r $REF_FASTA --algo GVCFtyper ${JOINT_VCF} ${SAMPLE_PREFIX}*.g.vcf
		...
		$RESULTS_DIR $REFERENCE_DIR $REF_FASTA $NUMBER_THREADS $SAMPLE_PREFIX) )
	fi
	
	    sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
		-e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
		${PIPELINE_DIR}/test_submit_all.sh --step5 ${OPTIONS[@]}
fi
...
if [ $STEP -eq 5 ]; then
	...
	TEMP_ARRAY_INCREMENT=1000
	SAMPLE_ARRAY=( $(find ${RESULTS_DIR} -maxdepth 1 -name "${SAMPLE_PREFIX}*merged.vcf.gz" ! -name "*multianno*" -exec basename {} \;) )
	JOB_COUNT=${#SAMPLE_ARRAY[@]} 
	TEMP_ARRAY_START=1
	TEMP_SAMPLE_ARRAY=( ${SAMPLE_ARRAY[@]:$(($TEMP_ARRAY_START - 1)):$TEMP_ARRAY_INCREMENT} )
	echo "temp sample array is :"${TEMP_SAMPLE_ARRAY}
	...
	DEPENDENCIES+=( $(sbatch --array=1-${TEMP_JOB_COUNT} --cpus-per-task $NUMBER_THREADS --parsable -e $STD_ERR_OUT_DIR/%A_annotate_%x.err -o $STD_ERR_OUT_DIR/%A_annotate_%x.out ${PIPELINE_DIR}/5_annovar.sh \
	...
	sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
                -e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
                ${PIPELINE_DIR}/test_submit_all.sh --step6 ${OPTIONS[@]}
...
if [ $STEP -eq 6 ]; then
	...
	TEMP_ARRAY_INCREMENT=1000
        ...
        DEPENDENCIES+=( $(sbatch --array=1-${TEMP_JOB_COUNT} --cpus-per-task $NUMBER_THREADS --parsable -e $STD_ERR_OUT_DIR/%A_manta_%x.err -o $STD_ERR_OUT_DIR/%A_manta_%x.out ${PIPELINE_DIR}/6_manta_sv.sh \
                $TEMP_SAMPLES_STRING $REF_FASTA $RESULTS_DIR $FINAL_DIR) )
	sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
			-e ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_test_submit_all_%x.out \
			${PIPELINE_DIR}/test_submit_all.sh --step7 ${OPTIONS[@]}
...
echo "### test_submit_all cycle done ### Finished: $(date)" >> $PIPELINE_STATUS
