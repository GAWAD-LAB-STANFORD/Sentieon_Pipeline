...
ml purge
ml php/7.3.0 ghostscript/9.53.2 gsl/2.3 R/4.0.2 biology bedtools/2.27.1 samtools/1.8
...
mkdir -p $RESULTS_DIR
mv ${FULL_WORK_DIR}/* ${RESULTS_DIR}/ginkgo_outputs/
if [ "$NEW_5M_DIR" != "0" ]; then
    mkdir -p $NEW_5M_DIR
    for SAMPLE in ${SAMPLE_ARRAY[@]}; do
        mv ${BAM_DIR}/${SAMPLE}${BAM_SUFFIX} ${NEW_5M_DIR}/
        mv ${BAM_DIR}/${SAMPLE}${BAM_SUFFIX}.bai ${NEW_5M_DIR}/
    done
fi
