...
#Have to put OPTIONS+= any arguments you want to continually be used through cycles of submit_all.sh
...
echo TEMP PIPELINE DIR IS ${TEMP_PIPELINE_DIR}
...
#		    fi
	...
	#Prep sample array for sentieon_BAM_construction.sh and setup to queue mutlipe paralell jobs into slurm for each sample    
		...
		SAMPLE_ARRAY=( $(find ${FASTQ_DIR} -maxdepth 1 -name "${SAMPLE_PREFIX}*${R1_SUFFIX}" ! -name "Undetermined*" -exec basename {} \; | grep -v "Undetermined" | sed "s/${R1_SUFFIX}//") )
	    fi
	    ...
	    DEPENDENCIES+=( $(sbatch --parsable -e $STD_ERR_OUT_DIR/%A_bam_construction_%x.err -o $STD_ERR_OUT_DIR/%A_bam_construction_%x.out \
		--array=1-${TEMP_JOB_COUNT} -p cgawad ${PIPELINE_DIR}/1_sentieon_BAM_construction.sh \
		$RESULTS_DIR $SKIP_TRIMMOMATIC $SCRIPT_DIR $TOOLS_DIR $R1_SUFFIX $R2_SUFFIX $REF_FASTA $NUMBER_THREADS $TEMP_SAMPLES_STRING $FASTQ_DIR $DBSNP_VCF $PROJECT $SKIP_BAM $SKIP_QC $TARGETED $STD_ERR_OUT_DIR $TARGETS_BED) )
...
#	    fi
		...
		sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
		    -e ${STD_ERR_OUT_DIR}/%A_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_submit_all_%x.out \
		    ${PIPELINE_DIR}/submit_all.sh --stepQC ${OPTIONS[@]}
	fi
fi
...
#	    echo "temp sample string is ${TEMP_SAMPLES_STRING}"
		...
		$RESULTS_DIR $SKIP_TRIMMOMATIC $SCRIPT_DIR $TOOLS_DIR $R1_SUFFIX $R2_SUFFIX $REF_FASTA $NUMBER_THREADS $TEMP_SAMPLES_STRING $DBSNP_VCF" >> $PIPELINE_STATUS
	    DEPENDENCIES+=( $(sbatch --parsable -e $STD_ERR_OUT_DIR/%A_calc_metrics_%a.err -o $STD_ERR_OUT_DIR/%A_calc_metrics_%a.out \
...
if [ $STEP -eq 2 ] && [ $SCAN2 -eq 1 ]; then
	...
	sbatch --time=7-00:00:00 --parsable -e $STD_ERR_OUT_DIR/%A_scan2_%x.err -o $STD_ERR_OUT_DIR/%A_scan2_%x.out ${PIPELINE_DIR}/2_scan2.sh --project $PROJECT --results_dir $RESULTS_DIR --genome_version $ANNOVAR_GENOME_VERSION --script_dir $SCRIPT_DIR --sample_prefix $SAMPLE_PREFIX --normal_path $NORMAL_PATH --sample_string $TEMP_SAMPLES_STRING --vcf_path $VCF_PATH --std_err_out $STD_ERR_OUT_DIR --normal_name $NORMAL_SAMPLE_NAME  --pipeline_dir $PIPELINE_DIR --final_dir $FINAL_DIR --targeted $TARGETED --targets_bed $TARGETS_BED --interval_list $INTERVAL_LIST --annovar_dir $ANNOVAR_DIR --email $EMAIL --skip_panel $SKIP_PANEL --cross_dir $CROSS_SAMPLE_DIR
 	#sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
        #            -e ${STD_ERR_OUT_DIR}/%A_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_submit_all_%x.out \
        #            ${PIPELINE_DIR}/submit_all.sh --step3 ${OPTIONS[@]} --array=1-${TEMP_JOB_COUNT}
...
if [ $STEP -eq 3 ]; then
	...
	echo "options are ${OPTIONS[@]}"
        ...
        echo "dependencies are: ${DEPENDENCIES[*]}" 
        DEPENDENCIES+=( $(sbatch  --parsable -e $STD_ERR_OUT_DIR/%A_germline_variant_call_%a.err -o $STD_ERR_OUT_DIR/%A_germline_variant_call_%a.out \
        --array=1-${TEMP_JOB_COUNT} ${PIPELINE_DIR}/3_germline_variant_calling.sh  \
        $RESULTS_DIR $REFERENCE_DIR $REF_FASTA $NUMBER_THREADS $TEMP_SAMPLES_STRING $TARGETS_BED) )
	...
	TEMP_ARRAY_INCREMENT=1000
	SAMPLE_ARRAY=( $(find ${RESULTS_DIR} -maxdepth 1 -name "${SAMPLE_PREFIX}*.realigned_deduped_sorted.bam" ! -name "*${NORMAL_SAMPLE_NAME}*" -exec basename {} \;) )
	JOB_COUNT=${#SAMPLE_ARRAY[@]} 
	TEMP_ARRAY_START=1
	TEMP_SAMPLE_ARRAY=( ${SAMPLE_ARRAY[@]:$(($TEMP_ARRAY_START - 1)):$TEMP_ARRAY_INCREMENT} )
	echo "temp sample array is :"${TEMP_SAMPLE_ARRAY}
	...
	DEPENDENCIES+=( $(sbatch --parsable -e $STD_ERR_OUT_DIR/%A_somatic_variant_call_%a.err -o $STD_ERR_OUT_DIR/%A_somatic_variant_call_%a.out \
	--array=1-${TEMP_JOB_COUNT} ${PIPELINE_DIR}/3_somatic_variant_calling.sh  \
	$RESULTS_DIR $REFERENCE_DIR $REF_FASTA $NUMBER_THREADS $NORMAL_SAMPLE_NAME $TEMP_SAMPLES_STRING $DBSNP_VCF $TARGETS_BED) )
...
if [ $STEP -eq 4 ]; then
	...
	if [ -n $NORMAL_SAMPLE_NAME ] && [ -n "$(find -name "${SAMPLE_PREFIX}*_variant.vcf")" ]; then
		...
		DEPENDENCIES+=( $(sbatch  -e ${STD_ERR_OUT_DIR}/%A_somatic_merge_%x.err -o ${STD_ERR_OUT_DIR}/%A_somatic_merge_%x.out --parsable  ${PIPELINE_DIR}/4_vcf_concat.sh $RESULTS_DIR $PROJECT $SAMPLE_PREFIX) )
	fi
...
#		sentieon driver -r $REF_FASTA --algo GVCFtyper ${JOINT_VCF} ${SAMPLE_PREFIX}*.g.vcf
		...
		DEPENDENCIES+=( $(sbatch --parsable -e ${STD_ERR_OUT_DIR}/%A_joint_genotyping_%x.err -o ${STD_ERR_OUT_DIR}/%A_joint_genotyping_%x.out \
			${PIPELINE_DIR}/4_joint_genotyping.sh \
			$RESULTS_DIR $REFERENCE_DIR $REF_FASTA $NUMBER_THREADS $PROJECT $TARGETS_BED) )
	fi
 	    if [ $ONLY_STEP -eq 1 ]; then
                echo "--only_step argument given, exiting"
                exit
...
if [ $STEP -eq 5 ]; then
	...
	TEMP_ARRAY_INCREMENT=1000
	SAMPLE_ARRAY=( $(find ${RESULTS_DIR} -maxdepth 1 -name "*merged.vcf.gz" ! -name "*multianno*" -exec basename {} \;) )
	JOB_COUNT=${#SAMPLE_ARRAY[@]}
	TEMP_ARRAY_START=1
	TEMP_SAMPLE_ARRAY=( ${SAMPLE_ARRAY[@]:$(($TEMP_ARRAY_START - 1)):$TEMP_ARRAY_INCREMENT} )
	echo "temp sample array is :"${TEMP_SAMPLE_ARRAY}
	...
	DEPENDENCIES+=( $(sbatch --array=1-${TEMP_JOB_COUNT} --parsable -e $STD_ERR_OUT_DIR/%A_annotate_%a.err -o $STD_ERR_OUT_DIR/%A_annotate_%a.out ${PIPELINE_DIR}/5_annovar.sh \
		$TEMP_SAMPLES_STRING $TRANCHE $PIPELINE_DIR $ANNOVAR_GENOME_VERSION $ANNOVAR_DIR $TOOLS_DIR $RESULTS_DIR $STD_ERR_OUT_DIR $REFERENCE_DIR $TARGETED $REF_FASTA $NORMAL_SAMPLE_NAME $EMAIL) )
 	    if [ $ONLY_STEP -eq 1 ]; then
                echo "--only_step argument given, exiting"
                exit
            fi
...
if [ $STEP -eq 6 ]; then
	...
	TEMP_ARRAY_INCREMENT=1000
        ...
        DEPENDENCIES+=( $(sbatch --parsable -e $STD_ERR_OUT_DIR/%A_6_filter_variants.err -o $STD_ERR_OUT_DIR/%A_6_filter_variants.err ${PIPELINE_DIR}/6_filter_annotated_variants.sh $PROJECT $RESULTS_DIR $EMAIL $PIPELINE_DIR $STD_ERR_OUT_DIR $TARGETED) )

	if [ $SKIP_MANTA -eq 0 ];then
        DEPENDENCIES+=( $(sbatch --array=1-${TEMP_JOB_COUNT} --parsable -e $STD_ERR_OUT_DIR/%A_manta_%a.err -o $STD_ERR_OUT_DIR/%A_manta_%a.out ${PIPELINE_DIR}/6_manta_sv.sh \
	...
	sbatch --dependency=afterany:$( IFS=$':'; echo "${DEPENDENCIES[*]}" ) -J $PROJECT \
			-e ${STD_ERR_OUT_DIR}/%A_submit_all_%x.err -o ${STD_ERR_OUT_DIR}/%A_submit_all_%x.out \
			${PIPELINE_DIR}/submit_all.sh --step7 ${OPTIONS[@]}
